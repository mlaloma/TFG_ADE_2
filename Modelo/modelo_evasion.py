...
def energia_intercambio(lattice, i, j, J):
    ...
    # Variación de la energía debido a la interacción entre espines
    dE = -J * S * vecinos  # Interacción de espines
    return dE
...
def on_key(event):
    ...
    if event.key == " ":  # Pausar/Reanudar con la barra espaciadora
        ...
        p_a = min(p_a + 0.01, 1.0)  # Aumentar hasta un máximo de 1
        ax.set_title(f"Modelo de Evasión Fiscal 2D\nTemperatura social: {T:.2f}\nProbabilidad de auditoría: {p_a:.2f}")

    elif event.key == "left":  # Disminuir probabilidad de auditoría
        p_a = max(p_a - 0.01, 0.0)  # Disminuir hasta un mínimo de 0
        ax.set_title(f"Modelo de Evasión Fiscal 2D\nTemperatura social: {T:.2f}\nProbabilidad de auditoría: {p_a:.2f}")
